...
import { central, ChartColors, northern } from 'src/app/constant/ChartColors';
...
@Component({
  selector: 'tpp-battery-backup-province',
  templateUrl: './battery-backup-province.component.html',
  styleUrls: ['./battery-backup-province.component.css']
})
export class BatteryBackupProvinceComponent implements OnInit {
  _region: string;
  filterOption: any = {
    region: '',
    province: [],
  ...
  set provinceInfo(totalProvince: any[]) {
    this.defaultProvince = JSON.parse(JSON.stringify(totalProvince));
  ...
  public Data: any[] = [];
  filterBatteryBackupProvinceModal: any;
  mobile: boolean;

  @ViewChild('canvasBackupProvince', { static: true }) canvasBackup: BaseChartDirective;
  ...
  async ngAfterViewInit() {
    this.filterBatteryBackupProvinceModal = new bootstrap.Modal(document.getElementById('filterBatteryBackupProvinceModal'));
  }
  ...
  searchData() {
    this.homeService.getBatteryBackupTimeProvince(this._region, this.filterOption)
      .subscribe(data => {
        this.pushLabel(data);
        this.pushData(data);
        this.filter();
  ...
  async openFilterModal() {
    this.filterBatteryBackupProvinceModal.show();
  }

  async selectProvinceFilterBatt(event: any, province: any) {
  ...
  async selectAllProvinceFilterBatt(event: any) {
    if (event.target.checked) {
  ...
  reset() {
    this._proviceInfo = JSON.parse(JSON.stringify(this.defaultProvince));
    this.filterMain = true;
    this.searchData();
  }

  dataChange(provinceId) {
    for (var i = 0; i < this._proviceInfo.length; i++) {
      if (this._proviceInfo[i].provinceId === provinceId) {
        this._proviceInfo[i].checked = !this._proviceInfo[i].checked;
      }
    }
    this.searchData();
  }
  ...
  filter() {
    let result = this._proviceInfo.filter(e => e.checked === false);
    result.forEach(e => {
      this.Data.forEach((element, index) => {
        let pName = e.provinceName.substr(0, 1).toUpperCase() + e.provinceName.substr(1);
        if (pName == element.label) this.Data.splice(index, 1);
      });
    });
    if (this.Data.length === 0) {
      this.filterMain = false;
    } else {
      this.filterMain = true;
    }
  }
  ...
  setProvince() {
    this.defaultProvince.forEach(function (value) {
      value['checked'] = true;
    });
    this._proviceInfo = JSON.parse(JSON.stringify(this.defaultProvince));
  }
  ...
  pushData(datas) {
    ...
    datas.forEach(d => {
      ...
      let pName = d.provinceName.substr(0, 1).toUpperCase() + d.provinceName.substr(1);
      let body = {
        label: pName,
        data: dataArray,
        type: 'line',
  ...
  removeData() {
    const removeItinerary = (region) => {
      const res = this.Data.filter(obj => obj.label !== region);
      return res;
    }
    // if (this.regionCheck.centralB == false)
    //   removeItinerary('central');
  }
